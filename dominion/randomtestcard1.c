...
int main (int argc, char** argv) {
  ...
  for (i = 0; i < 250; i++) { //run 250 tests
	  ...
	  G.hand[0][0] = council_room; //place desired card
	  playCard(0, -1, -1, -1, p); //play card
	  printf("Checking handcount\n");
	  myassert(G.handCount(0) == handsize + 3);
	  printf("Checking other handcount\n");
	  myassert(G.handCount(1) == 1);
	  printf("Checking buys\n");
	  myassert(G.numBuys == 2);
	  printf("Checking card discarded\n");
	  myassert(G.discard[0][discardsize(0)] == council_room);
	  if (!failed) {
